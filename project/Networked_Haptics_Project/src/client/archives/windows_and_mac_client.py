...
def send_message():
    ...
    try:
        ...
        tk.Label(top, text="Haptic Message",  font = ('calibri', 13, 'bold'), background=bg_color, fg='white').grid(row=0, column=0, padx=pad_x, pady=pad_y)
...
def create_activity():
    ...
    tk.Label(top, text="Message Name",  font = ('calibri', 13, 'bold'), background=bg_color, fg='white').grid(row=0, column=0, padx=pad_x, pady=pad_y)
    ...
    tk.Label(top, text="No of Vibration Motors",  font = ('calibri', 13, 'bold'), background=bg_color, fg='white').grid(row=1, column=0, padx=pad_x, pady=pad_y)
                    ...
                    if BUSY == 0:
                        if (response['tag'] == "send_rtdata"): 
                            ...
                            if resp == 1:
                                ...
                                MESSAGE_DELAY = int(total_durtn / 10 ) + MESSAGE_BUFFER_DELAY
                            else:
...
def read_send_from_arduino():
    ...
    try:
        if ser:
            if ( ser.inWaiting() > 0 ):
                data_str = ser.readline(ser.inWaiting()).decode('ascii').strip('\r\n') 
...
send_btn              = Button(frame, text="SEND A PRERECORDED HAPTIC MESSAGE", command=send_message, width=35) 
rt_btn                = Button(frame, text="REALTIME HAPTIC COMMUNICATION", command=send_rtmessage, width=35)
rt_self_btn           = Button(frame, text="REALTIME SELF TOUCH", command=self_touch, width=35)
create_actvty_btn     = Button(frame, text="PRE-RECORD A HAPTIC MESSAGE", command=create_activity, width=35)
port_btn              = Button(frame, text="ENTER ARDUINO PORT NO", command=get_port, width=35)
create_accnt_btn      = Button(frame, text="CREATE NEW USER ACCOUNT", command=create_account_window, width=35)
# server_addr_btn       = Button(frame, text="ENTER SERVER ADDRESS", command=get_serv_addr, width=35)  
server_addr_btn       = Button(frame, text="SEND TEST MESSAGE", command=send_test_message, width=35) 
